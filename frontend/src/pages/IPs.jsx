...
export default function IPs() {
  ...
  // Ping 1 IP
  async function handlePingOne(row) {
    if (!row.ip) return
    setPingingId(row.id)
    try {
      const res = await api.post('/ping/ip/check', { ip: row.ip })
      setPingResults(prev => ({ ...prev, [row.id]: res.data }))
    } catch { setPingResults(prev => ({ ...prev, [row.id]: { status: 'offline', latency: null } })) }
        ...
        <table className="data-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={8} className="text-center py-12 text-[13px] text-[#97a0af]">Không có dữ liệu</td></tr>
            ) : rows.map((row, i) => {
              const pr = pingResults[row.id]
              return (
                <tr key={row.id} className="tr-hover group">
                  ...
                  <td className="td text-center">
                    {pingingId === row.id ? (
                      <div className="inline-flex items-center gap-1 text-[11px] text-[#6b778c]">
